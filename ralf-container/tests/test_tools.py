...
def test_file_tools():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = write_file.invoke({"path": file_path, "content": "Hello World"})
        ...
        # Test list_files
        files = list_files.invoke({"path": tmpdir})
        assert "test.txt" in files

        # Test read_file non-existent
        result = read_file.invoke({"path": os.path.join(tmpdir, "nonexistent.txt")})
...
def test_run_command():
    ...
    result = run_command.invoke({"command": "ls /nonexistent_directory_xyz"})
